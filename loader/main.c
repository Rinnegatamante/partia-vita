...
so_module partia_mod, stdcpp_mod;
...
int debugPrintf(char *text, ...) {
	...
	SceUID fd = sceIoOpen("ux0:data/partia_log.txt", SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0777);
	if (fd >= 0) {
...
Mix_Chunk *audio_effects[64];
Mix_Music *music = NULL;
int LoadEffect(char *name, unsigned int id) {
	audio_effects[id] = Mix_LoadWAV(name);
	return audio_effects[id] ? 1 : 0;
}

void PlaySound(void *unk, unsigned int id, float unk2) {
	Mix_PlayChannel(-1, audio_effects[id], 0);
}

void PlayMusic(void *unk, const char *name, float unk2) {
	if (music) {
		Mix_HaltMusic();
		Mix_FreeMusic(music);
	}
	char fname[256];
	sprintf(fname, "%s/assets/sound/%s", DATA_PATH, name);
...
static so_default_dynlib default_dynlib[] = {
	...
	{ "SDL_CreateRenderer", (uintptr_t)&SDL_CreateRenderer_hook },
...
int main(int argc, char *argv[]) {
	...
	*(uintptr_t *)(fake_env + 0x374) = (uintptr_t)GetStringUTFRegion;
	...
	int (* SDL_main)(void) = (void *) so_symbol(&partia_mod, "SDL_main");
    SDL_main();
